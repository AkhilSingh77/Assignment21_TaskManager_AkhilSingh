...
const duedateInputElement = document.querySelector(
  ".duePriorityContainer__left--input"
) as HTMLInputElement;
const priorityInputElement = document.querySelector(
  ".duePriorityContainer__right__select"
) as HTMLInputElement;
const categoryInputElement = document.querySelector(
  ".categoryContainer__inputContainer__select"
) as HTMLInputElement;
const addbuttonElement = document.querySelector(
  ".leftwrapper__form--button"
) as HTMLButtonElement;
const leftPartMesaageElement = document.querySelector(
  ".mainwrapper__left--message"
) as HTMLDivElement;
const taskContainerElement = document.querySelector(
  ".mainwrapper__right__taskContainer "
) as HTMLDivElement;
const tagsContainerElement = document.querySelector(
  ".categoryContainer__inputContainer__tagContainer"
) as HTMLDivElement;
const searchElement = document.querySelector(
  ".topContainer__searchContainer--input"
) as HTMLInputElement;
const selectPriorityElementRight = document.querySelector(
  ".topContainer__bottom__sortContainer__select"
) as HTMLSelectElement;

const filterInputElementRight = document.querySelector(
  ".topContainer__bottom__filterContainer__select"
) as HTMLSelectElement;

const filterTagContainerRight = document.querySelector(
  ".filterContainer__tagcontainer"
) as HTMLDivElement;

const selectCategoryRightElement = document.querySelector(
  ".filterContainer__select"
) as HTMLSelectElement;
...
let catagoryDeleted: boolean = false;
...
function searchTaskFunction() {
  ...
  const afterSearchFilteredArray = alltaskArray.filter((card) => {
    ...
    if (
      taskName.includes(valueInInput) &&
      descriptionDetail.includes(valueInInput)
    ) {
      return true;
  ...
  if (categoryValue !== "") {
    ...
    if (categoryRightArray.length !== 0) {
      ...
      const afterTagsArray = afterSortPriorityArray.filter((task) =>
        task.category.some(
          (category) => categoryRightArray.indexOf(category) !== -1
        )
      );
...
function filterTaskFunction(afterSearchArray: Task[]) {
  ...
  if (filterValue === "Completed") {
    const completedTaskArray = afterSearchArray.filter(
      (task) => task.isChecked
    );
...
function createCards(element: Task) {
  ...
  middlePart.className =
    "taskContainer__singleTaskContainer__middle middlePart";
  ...
  deleteIcon.addEventListener("click", () => {
    const indexOfTaskInArray = alltaskArray.findIndex(
  ...
  taskContainerElement.insertBefore(
    singleTaskContainer,
    taskContainerElement.firstChild
  );
  checkbox.addEventListener("click", () => {
    if (checkbox.checked) {
      heading.classList.add("strikethrough");

      const indexOfElementInAlltask = alltaskArray.findIndex(
        (arr) => arr.id === element.id
      );
...
function createTag(tagName: string) {
  ...
  tagContainer.classList.add(
    "categoryContainer__inputContainer__tagContainer__tag",
    "tag"
  );
...
function runAllFilterTogether() {
  ...
  const afterCategoryArray = catagoryDeleted
    ? afterDeleetetagsCard(afterSortPriorityArray)
    : categoryFilterFunction(afterSortPriorityArray);

  traverseCardsArray(afterCategoryArray);
}

function afterDeleetetagsCard(afterSortPriorityArray: Task[]) {
  filterTagContainerRight.innerHTML = "";
  if (categoryRightArray.length !== 0) {
    categoryRightArray.forEach((tag) => {
      createTagsRight(tag);
